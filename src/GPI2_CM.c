...
gaspi_return_t
pgaspi_connect_endpoint_to (const gaspi_rank_t rank,
                            const gaspi_timeout_t timeout_ms)
{
  const int i = (int) rank;
...
gaspi_return_t
pgaspi_local_disconnect (const gaspi_rank_t rank,
                         const gaspi_timeout_t timeout_ms)
{
  const int i = rank;
  gaspi_return_t eret = GASPI_ERROR;
...
pgaspi_disconnect (const gaspi_rank_t rank, const gaspi_timeout_t timeout_ms)
{
  ...
  /* TODO: we can still get trapped inside the sn command, trying to
   * connect to a remote rank and in case the remote rank is
